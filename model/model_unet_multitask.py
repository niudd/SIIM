...
from metrics import iou_pytorch, dice, dice_multitask
...
class UNetResNet34(nn.Module):
    ...
    def criterion(self, logit_mask, truth_mask, logit_clf, truth_clf):
        ...
        loss_mask_nonempty = (loss_mask.mean(dim=[2,3])*truth_clf).mean() #loss for empty image is weighted 0

        loss_clf = Loss_FUNC_clf(logit_clf, truth_clf)
        loss = 0.95*loss_mask_nonempty + 0.05*loss_clf
