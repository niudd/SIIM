...
#             param[1].requires_grad = False
    ...
    for i_epoch in range(NUM_EPOCHS):
        ...
        for i, (image, masks) in enumerate(train_dl):
            ...
            #set_trace()
            logit, logit_mask, logit_clf = net(input_data)#[:, :3, :, :]
            
            if multi_gpu is not None:
                _train_loss = net.module.criterion(logit, logit_mask, logit_clf, truth_mask, truth_clf)
                _train_metric = net.module.metric(logit, truth_mask)#device='gpu'
            else:
                _train_loss = net.criterion(logit, logit_mask, logit_clf, truth_mask, truth_clf)
                _train_metric  = net.metric(logit, truth_mask)#device='gpu'
            train_loss_list.append(_train_loss.item())
            train_metric_list.append(_train_metric.item())#.detach()

            #grandient accumulation step=4, batch_size=4; step=2, batch_size=8
            acc_step = 2
        ...
        with torch.no_grad():
            ...
            for i, (image, masks) in enumerate(val_dl):
                ...
                if multi_gpu is not None:
                    _val_loss = net.module.criterion(logit, logit_mask, logit_clf, truth_mask, truth_clf)
                    _val_metric = net.module.metric(logit, truth_mask)#device='gpu'
                else:
                    _val_loss = net.criterion(logit, logit_mask, logit_clf, truth_mask, truth_clf)
                    _val_metric = net.metric(logit, truth_mask)#device='gpu'
...
#             logit_valid, logit_mask_valid, logit_clf_valid, truth_mask_valid, truth_clf_valid = None, None, None, None, None
#             for j, (image, masks) in enumerate(val_dl):
#                 input_data = image.to(device=device, dtype=torch.float)
#                 truth_mask = masks.cpu().float()
#                 #define non-empty-mask as 1
#                 truth_clf = (truth_mask.sum(dim=2).sum(dim=2)!=0).cpu().float()
#                 logit, logit_mask, logit_clf = net(input_data)
#                 logit, logit_mask, logit_clf = logit.cpu().float(), logit_mask.cpu().float(), logit_clf.cpu().float()
...
#             if multi_gpu is not None:
#                 val_loss = net.module.criterion(logit_valid, logit_mask_valid, logit_clf_valid, truth_mask_valid, truth_clf_valid)
#                 val_metric = net.module.metric(logit_valid, truth_mask_valid)
#             else:
#                 val_loss, val_loss_mask, val_loss_clf = net.criterion(logit_valid, logit_mask_valid, logit_clf_valid, truth_mask_valid, truth_clf_valid)
#                 val_metric = net.metric(logit_valid, truth_mask_valid)
...
IMG_SIZE = 512
BATCH_SIZE = 8 #accumulation=2
NUM_WORKERS = 24
warm_start, last_checkpoint_path = True, 'checkpoint/deep_supervision_%s_%s_v1_seed%s/best.pth.tar'%(MODEL, IMG_SIZE, SEED)
