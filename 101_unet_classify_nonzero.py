...
#             param[1].requires_grad = False
    ...
    for i_epoch in range(NUM_EPOCHS):
        ...
        if multi_gpu is not None:
            train_metric = net.module.metric(torch.cat(logit_list, dim=0), torch.cat(truth_list, dim=0))
        else:
            train_metric = net.metric(torch.cat(logit_list, dim=0), torch.cat(truth_list, dim=0))
