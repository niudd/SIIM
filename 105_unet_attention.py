...
from model.model_unet_attention import R2AttU_Net, predict_proba
from model.model_unet_attention.lr_scheduler import LR_Scheduler
...
def run_check_net(train_dl, val_dl, multi_gpu=[0, 1]):
    ...
    #---
    if MODEL == 'R2AttU_Net':
        net = R2AttU_Net(debug=False).cuda(device=device)
...
#             param[1].requires_grad = False
    ...
    #customized
    optimizer = torch.optim.SGD(train_params, lr=LearningRate, momentum=0.9, weight_decay=0.0001)
